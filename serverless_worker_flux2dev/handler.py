...
Requires H100/A100 80GB GPU. Supports negative_prompt (unlike Klein).
"""
...
def handler(job: dict) -> dict:
    ...
    # Build full prompt — append anatomy guard for better human figures
    full_prompt = f"{prompt}, {global_style}".strip(", ") if global_style else prompt

    # Default negative prompt if none provided
    if not negative_prompt:
        negative_prompt = "extra limbs, deformed anatomy, extra arms, extra legs, mutation, disfigured"
...
# Start the serverless loop IMMEDIATELY — model lazy-loads on first job.
