...
from vast_worker import server
...
def test_generate_rejects_missing_worker_token(monkeypatch) -> None:
    load_model = MagicMock(return_value=_FakePipe())
    monkeypatch.setattr(server, "_load_model", load_model)
    client = TestClient(server.app)
    resp = client.post(
        "/generate",
        json={
...
def test_generate_uses_clip_and_full_prompt_contract(monkeypatch) -> None:
    pipe = _FakePipe()
    monkeypatch.setattr(server, "_load_model", lambda: pipe)
    client = TestClient(server.app)
    resp = client.post(
        "/generate",
...
def test_model_revision_preflight_rejects_bad_values(monkeypatch, revision: str) -> None:
    ...
    with pytest.raises(RuntimeError, match="pinned"):
        server._validate_model_revision()
